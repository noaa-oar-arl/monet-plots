...
* **Consistency:** All plots inherit from [`BasePlot`](https://github.com/noaa-oar-arl/monet-plots/blob/main/src/monet_plots/plots/base.py) to enforce consistent styling (via `wiley_style`) and standard figure lifecycle management (`__init__`, `save`, `close`).
* **Decoupling:** Statistical calculation logic is entirely decoupled from visualization logic, residing in the utility module [`verification_metrics.py`](https://github.com/noaa-oar-arl/monet-plots/blob/main/src/monet_plots/verification_metrics.py).
...
All verification plots extend the abstract base class [`BasePlot`](https://github.com/noaa-oar-arl/monet-plots/blob/main/src/monet_plots/plots/base.py), providing standardized figure handling.
...
| :--- | :--- |
| [`src/monet_plots/plots/base.py`](https://github.com/noaa-oar-arl/monet-plots/blob/main/src/monet_plots/plots/base.py) | **Foundation**: Defines [`BasePlot`](https://github.com/noaa-oar-arl/monet-plots/blob/main/src/monet_plots/plots/base.py). Enforces standard figure management and styling. |
| Plot-Specific Classes (e.g., [`roc_curve.py`](https://github.com/noaa-oar-arl/monet-plots/blob/main/src/monet_plots/plots/roc_curve.py)) | **Visualization**: Inherit from `BasePlot`. Handle plot-specific data wrangling (e.g., binning, feature selection) and Matplotlib rendering using the `plot()` method. |
| [`src/monet_plots/verification_metrics.py`](https://github.com/noaa-oar-arl/monet-plots/blob/main/src/monet_plots/verification_metrics.py) | **Computation**: Contains pure, stateless mathematical functions (e.g., `compute_pod`, `compute_brier_score_components`). This module only depends on `numpy` and `pandas`, not Matplotlib. |

## 5. Integration Points with `verification_metrics.py`

The integration point is strictly from the concrete plot classes back to the stateless functions in [`verification_metrics.py`](https://github.com/noaa-oar-arl/monet-plots/blob/main/src/monet_plots/verification_metrics.py).

* **Data Preparation**: Plot classes ingest raw or pre-processed data (often in a Pandas DataFrame).
* **Metric Calculation**: Within the plot's primary rendering method (usually `plot()` or a helper), relevant data columns are passed as arguments to the functions in [`verification_metrics.py`](https://github.com/noaa-oar-arl/monet-plots/blob/main/src/monet_plots/verification_metrics.py) (e.g., passing `forecasts` and `observations` to [`compute_reliability_curve`](https://github.com/noaa-oar-arl/monet-plots/blob/main/src/monet_plots/verification_metrics.py)).
...
1. **Inheritance:** Create a new class that inherits directly from [`BasePlot`](https://github.com/noaa-oar-arl/monet-plots/blob/main/src/monet_plots/plots/base.py).
2. **Method Overriding:** Implement the core rendering logic within a new `plot(...)` method in the new class. This method is responsible for calling utility functions from [`verification_metrics.py`](https://github.com/noaa-oar-arl/monet-plots/blob/main/src/monet_plots/verification_metrics.py) as needed.
...
4. **Dependency Management:** New plot classes should avoid introducing new, unmanaged external dependencies unless absolutely necessary. If new statistical methods are needed, they should be added as new functions to [`verification_metrics.py`](https://github.com/noaa-oar-arl/monet-plots/blob/main/src/monet_plots/verification_metrics.py) first to maintain the computational/visualization split.
